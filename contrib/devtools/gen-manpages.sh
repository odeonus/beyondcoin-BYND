...
BEYONDCOIND=${BEYONDCOIND:-$SRCDIR/beyondcoind}
BEYONDCOINCLI=${BEYONDCOINCLI:-$SRCDIR/beyondcoin-cli}
BEYONDCOINTX=${BEYONDCOINTX:-$SRCDIR/beyondcoin-tx}
BEYONDCOINQT=${BEYONDCOINQT:-$SRCDIR/qt/beyondcoin-qt}

[ ! -x $BEYONDCOIND ] && echo "$BEYONDCOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
LTCVER=($($BEYONDCOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
