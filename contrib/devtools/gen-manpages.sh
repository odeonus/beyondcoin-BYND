...
LITECOIND=${LITECOIND:-$SRCDIR/beyondcoind}
LITECOINCLI=${LITECOINCLI:-$SRCDIR/beyondcoin-cli}
LITECOINTX=${LITECOINTX:-$SRCDIR/beyondcoin-tx}
LITECOINQT=${LITECOINQT:-$SRCDIR/qt/beyondcoin-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
BYNDVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
