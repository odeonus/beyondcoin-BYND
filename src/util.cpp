...
const char * const BITCOIN_CONF_FILENAME = "beyondcoin.conf";
const char * const BITCOIN_PID_FILENAME = "beyondcoind.pid";
...
#else
    const char* pszModule = "beyondcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Beyondcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Beyondcoin";
#else
    // Unix
    return pathRet / ".beyondcoin";
#endif
#endif
}
