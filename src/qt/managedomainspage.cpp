...
void ManageNamesPage::setModel(WalletModel *walletModel)
{
    ...
    model = walletModel->getDomainTableModel();
...
{
    GUIUtil::copyEntryData(ui->tableView, DomainTableModel::Name);
}

void ManageNamesPage::onCopyValueAction()
{
    GUIUtil::copyEntryData(ui->tableView, DomainTableModel::Value);
}
...
void ManageNamesPage::on_configureNameButton_clicked()
{
    if (!ui->tableView->selectionModel())
        return;
    const QModelIndexList &indexes = ui->tableView->selectionModel()->selectedRows(DomainTableModel::Name);
    if (indexes.isEmpty())
        return;
    ...
    const QString &value = index.sibling(index.row(), DomainTableModel::Value).data(Qt::EditRole).toString();
...
void ManageNamesPage::exportClicked()
{
    ...
    writer.addColumn("Name", DomainTableModel::Name, Qt::EditRole);
    writer.addColumn("Value", DomainTableModel::Value, Qt::EditRole);
    writer.addColumn("Expires In", DomainTableModel::ExpiresIn, Qt::EditRole);
